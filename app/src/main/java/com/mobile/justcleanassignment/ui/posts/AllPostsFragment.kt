...
import com.mobile.justcleanassignment.ui.postdetails.PostDetailsFragment.Companion.ARGS_POST
...
class AllPostsFragment : BaseFragment() {
    ...
    private fun observeViewModel() {
        postsViewModel.postsMutableLiveData.observe(viewLifecycleOwner, { res ->
            when (res.status) {
                ApiStatus.LOADING -> {
                    if (::spotsDialog.isInitialized && !postsViewModel.isLoadedFirstTime) {
                        postsViewModel.isLoadedFirstTime = true
                        spotsDialog.show()
                    }
                }
                ApiStatus.SUCCESS -> {
